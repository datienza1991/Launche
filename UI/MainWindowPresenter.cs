...
namespace UI.MainWindowx.Presenter
{
    ...
    public class MainWindowPresenter
    {
        ...
        {
            var result = await this.presenter.SortDownProjectService!.Handle(new() { SortId = this.presenter.MainWindowViewModel!.SelectedProjectPath!.SortId });
            if (!result)
            {
                return;
            }
        ...
        {
            if (presenter.ProjectPathsListView.SelectedIndex == -1) { return; }
            var project = (ProjectViewModel)presenter.ProjectPathsListView.SelectedItem;
            var currentGitBranch = presenter.GetCurrentGitBranchService!.Handle(new() { DirectoryPath = project.Path });
            ...
            this.presenter.MainWindowViewModel!.SelectedIdePath = this.presenter.MainWindowViewModel!.IdePathsModels!.First(x => x.Id == project.IDEPathId);
        }

        private async void Presenter_SortUpProjectEvent(object? sender, EventArgs e)
        {
            var result = await this.presenter.SortUpProjectService!.Handle(new() { SortId = this.presenter.MainWindowViewModel!.SelectedProjectPath!.SortId });
            if (!result)
            {
                return;
            }
        ...
        {
            var searchViewModel = await this.presenter.SearchProjectService!.Handle(new() { Search = this.presenter.MainWindowViewModel!.Search });
            this.presenter.MainWindowViewModel.EnableAddNewProject = searchViewModel.EnableAddNewProject;
